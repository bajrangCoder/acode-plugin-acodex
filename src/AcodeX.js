...
const FONT_CDN_BASE = "https://cdn.jsdelivr.net/gh/bajrangCoder/acode-plugin-acodex@main/fonts/";
...
export default class AcodeX {
  ...
  constructor() {
    if (!appSettings.value[plugin.id]) {
      ...
      if (!Object.prototype.hasOwnProperty.call(this.settings, "fontLigatures")) {
    ...
    this.handleStartDraggingFloatingBtn = this.startDraggingFloatingBtn.bind(this);
    ...
    this.handleToggleTerminalCommand = this.handleToggleTerminalCommand.bind(this);
  ...
  renderUI() {
    ...
    this.$showTermBtn.dataset.state = this.isTerminalMinimized ? "minimized" : "hidden";
  }
  ...
  handleResize() {
    if (this.$terminalContainer && !this.$terminalContainer.classList.contains("hide")) {
      ...
      const totalFooterHeight = document.querySelector("#quick-tools")?.offsetHeight || 0;
    ...
    if (
      ...
      const fileTabHeight = document.querySelector("#root ul")?.offsetHeight || 0;
      const totalHeaderHeight = headerHeight + fileTabHeight;
      const maxY = window.innerHeight - totalHeaderHeight - this.$showTermBtn.offsetHeight;
  ...
  async init($page, cacheFile, cacheFileUrl) {
    try {
      ...
      this.$terminalHeader.addEventListener("mousedown", this.handleStartDragging);
      ...
      if (app.get("main")) {
        app
          .get("main")
          .append(this.$terminalContainer, this.settings.showTerminalBtn ? this.$showTermBtn : "");
      }
      ...
      this.$showTermBtn.addEventListener("mousedown", this.handleStartDraggingFloatingBtn);
      this.$showTermBtn.addEventListener("touchstart", this.handleStartDraggingFloatingBtn);
      ...
      acode.define("acodex", {
         ...
         */
        openTerminal: async (termContainerHeight = 270, port = this.settings.port) => {
          if (!this.isTerminalOpened) {
            const socket = await this.openTerminalPanel(termContainerHeight, port);
            return {
              /**
              ...
              onmessage: (cb) => {
                if (socket) {
                  socket.onmessage = (event) =>
                    cb(typeof event.data === "string" ? event.data : new Uint8Array(event.data));
                }
              },
              /**
  ...
  async openTerminalPanel(termContainerHeight, port) {
    ...
    if (this.settings.transparency) {
      ...
      this.$terminalHeader.style.background = helpers.transparentColor(this.$terminalHeader);
      this.$terminalHeader.style.backdropFilter = `blur(${this.settings.blurValue})`;
    } else {
      this.$terminalContainer.style.background = "var(--popup-background-color)";
  ...
  async openAIPromptPopup() {
    const aiResponseHandler = new AIResponseHandler(
      this.settings.aiModel === AVAILABLE_AI_MODELS[3][0] ? null : this.settings.aiApiKey,
    );
    let ollamaModel;
    ...
    promptBox.ok(async () => {
      ...
      try {
        ...
        switch (this.settings.aiModel) {
          case "deepseek": {
            const { response, error } = await aiResponseHandler.generateDeepseekResponse(prompt);
            if (error) {
              if (loaderElement) {
          ...
          case "local-llm": {
            const ollamaRes = await aiResponseHandler.generateOllamaResponse(ollamaModel, prompt);
  ...
  async createSession() {
    ...
    this._updateTerminalHeight();
    localStorage.setItem("AcodeX_Current_Session", sessionsData[sessionsData.length - 1].name);
    this.setUiState({
      sessionName: sessionsData[sessionsData.length - 1].name,
      searchVisible: false,
      searchQuery: "",
    });
    window.toast(`Created Session: ${sessionsData[sessionsData.length - 1].name}`, 3000);
    return this.socket;
  }
  ...
  async _generateProcessId() {
    try {
      ...
      const res = await fetch(`http://${this.terminalServerHost}:${this.settings.port}/terminals`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  _setTerminalPadding(padding) {
    ...
    const safePadding = Number.isNaN(numericPadding) ? 0 : Math.max(0, numericPadding);
  ...
  async _getLastSessionName() {
    try {
      ...
      console.error("Error: No sessions found in JSON file or sessions data is not an array.");
      return null;
    } catch (error) {
  ...
  async closeTerminal() {
    ...
    if (this.$terminal != null && localStorage.getItem("AcodeX_Current_Session")) {
  ...
  dragFlotButton(e) {
    try {
      ...
      const buttonBottom =
        window.innerHeight - (this.$showTermBtn.offsetTop + this.$showTermBtn.offsetHeight) + newY;
  ...
  startDragging(e) {
    ...
    this.$terminalContainer.style.borderTop = "1px solid var(--link-text-color)";
  }
  ...
  maxmise() {
     ...
     */
    if (this.isTerminalMinimized) {
      if (Number.parseInt(localStorage.getItem("AcodeX_Terminal_Cont_Height")) <= 50) {
  ...
  async destroy() {
    ...
    this.$showTermBtn?.removeEventListener("mousedown", this.handleStartDraggingFloatingBtn);
    this.$showTermBtn?.removeEventListener("touchstart", this.handleStartDraggingFloatingBtn);
  ...
  async setCustomFontFile() {
    const { url } = await acode.fileBrowser("file", "select custom font stylesheet");
    if (!url) return;
  ...
  get terminalServerHost() {
    return this.isBuiltinTerminalBackend ? "localhost" : this.settings.serverHost;
  }
  ...
  async ensureTerminalBackendReady() {
    ...
    if (
      ...
      throw new Error("Builtin terminal backend is unavailable in this Acode build.");
    }
