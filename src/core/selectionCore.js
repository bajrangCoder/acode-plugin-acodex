export default class SelectionCore {
  constructor(terminal, terminalContent, settings = {}, options = {}) {
  ...
  createSelectionOverlay() {
    ...
    this.selectionOverlay.style.cssText = `
      ...
      overflow: hidden;
    `;
    this.terminalContent.appendChild(this.selectionOverlay);
  }
  ...
  attachEventListeners() {
    ...
    this.terminal.element.addEventListener("touchstart", this.boundHandlers.terminalTouchStart, {
      passive: false,
    });
    this.terminal.element.addEventListener("touchmove", this.boundHandlers.terminalTouchMove, {
      passive: false,
    });
    this.terminal.element.addEventListener("touchend", this.boundHandlers.terminalTouchEnd, {
      passive: false,
    });
    ...
    this.startHandle.addEventListener("touchstart", this.boundHandlers.handleTouchStart, {
      passive: false,
    });
    this.startHandle.addEventListener("touchmove", this.boundHandlers.handleTouchMove, {
      passive: false,
    });
    this.startHandle.addEventListener("touchend", this.boundHandlers.handleTouchEnd, {
      passive: false,
    });

    this.endHandle.addEventListener("touchstart", this.boundHandlers.handleTouchStart, {
      passive: false,
    });
    this.endHandle.addEventListener("touchmove", this.boundHandlers.handleTouchMove, {
      passive: false,
    });
    this.endHandle.addEventListener("touchend", this.boundHandlers.handleTouchEnd, {
      passive: false,
    });
    ...
    this.scrollElement =
      this.terminal.element.querySelector(".xterm-viewport") || this.terminal.element;
    this.scrollElement.addEventListener("scroll", this.boundHandlers.terminalScroll, {
      passive: true,
    });
    ...
    this.boundHandlers.documentTouchStart = this.onDocumentTouchStart.bind(this);
    document.addEventListener("touchstart", this.boundHandlers.documentTouchStart, {
      passive: true,
    });
  }
  ...
  onTerminalTouchMove(event) {
    ...
    if (clearTouch) {
      ...
      if (clearDeltaX > this.options.moveThreshold || clearDeltaY > this.options.moveThreshold) {
  ...
  onHandleTouchStart(event) {
    ...
    if (!handleType) {
      if (event.target === this.startHandle || this.startHandle.contains(event.target)) {
        handleType = "start";
      } else if (event.target === this.endHandle || this.endHandle.contains(event.target)) {
        handleType = "end";
      }
    }
  ...
  onDocumentTouchStart(event) {
    ...
    if (this.terminalContent.contains(event.target)) return;
  ...
  setHandleOrientation(handle, orientation) {
    ...
    handle.style.transform = hasScale ? `${baseTransform} scale(1.2)` : baseTransform;
  }
  ...
  createContextMenu() {
    ...
    const menuItems = [
      ...
      {
        label: this.getString("select all", "Select All"),
        action: this.selectAllText.bind(this),
      },
    ];
  ...
  terminalCoordsToPixels(coords) {
    ...
    const y =
      (coords.row - this.terminal.buffer.active.viewportY) * this.cellDimensions.height +
  ...
  getWordBoundsAt(coords) {
    ...
    try {
      ...
      while (endCol < lineText.length - 1 && this.isWordCharacter(lineText[endCol + 1])) {
        endCol++;
      }
  ...
  destroy() {
    ...
    document.removeEventListener("touchstart", this.boundHandlers.documentTouchStart);
